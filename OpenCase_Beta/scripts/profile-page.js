        ...
        $(function () {
            ...
            $(document).on('click', '#send-trade', function() {
                ...
                $('#summ-your-offer li').each(function() {
                    ...
                    var weapon = getWeapon(inventoryID);
                    tradeWeapons.push(weapon);
                    ids.push(inventoryID);
                })
                var accepted = $('#trade-its-gift').is(':checked');
                var convertedWeapons = fbInventory.convertInventory(tradeWeapons);
                fbProfile.newTrade(uid, convertedWeapons, accepted, function() {
                    ...
                    Lobibox.notify('success', {
                        ...
                        , position: 'top center'
                        , icon: false
                        , title: Localization.profile2.trade_send_title[Settings.language]
                        , size: 'mini'
            ...
            function showMyTrades() {
                ...
                fbProfile.getMyTrades(function (trades) {
                    ...
                    for(var i = 0; i < trades.length; i++) {
                        ...
                        $('.my-trades-list').prepend("<li class='my-trades-list__tradeWith' data-uid='"+trades[i].tradeWith.uid+"' data-trades='"+tradesList+"'><div class='tradeWith__img-container'><img src='../images/ava/0.jpg'></div>" +
                        "<div class='tradeWith__text-container'><span class='tradeWith__nickname'>Loading...</span><span class='tradeWith__tradeCount'>"+Localization.profile2.exchange_offers[Settings.language]+trades[i].trades.length+"</span></div><div class='tradeWith__controls'><i aria-hidden='true' class='fa fa-times delete-trade'></i></div></li>");
            ...
            $(document).on('click', '.trades-with-user__trade', function() {
                ...
                if ($(this).data('youAccepted'))
                    $("#you-ready-to-trade").prop('checked', true);
                else
                    $("#you-ready-to-trade").prop('checked', false);
                
                if ($(this).data('otherAccepted')) {
                    $(".other-player-ready").text(Localization.trades2.otherReady[Settings.language]);
                    $(".other-player-ready").addClass('ready');
                } else {
                    $(".other-player-ready").text(Localization.trades2.otherNotReady[Settings.language]);
                ...
                .then(function (myWeaponsSnapshot) {
                    ...
                    for (var i = 0; i < myWeapons.length; i++) {
                        ...
                        weapon = new Weapon_FB(weapon.item_id, weapon.quality, weapon.stattrak, weapon.souvenir);
            ...
            $(document).on('click', '#change-weapons-trade', function() {
                ...
                $('#my-trades__your-offer .trade__info__weapons.your li').each(function() {
                    var weapon = $(this).data('weapon_obj');
                    oldWp.push(weapon);
                });
                if (oldWp.length >= maxItems)  {
                    Lobibox.notify('error', {
                        ...
                        , position: 'top center'
                        , icon: false
                        , title: Localization.trades2.toManyItemsTitle[Settings.language]
                        , size: 'mini'
                        , showClass: 'fadeInDown'
                        , hideClass: 'fadeOutUp'
                        , msg: Localization.trades2.toManyItems[Settings.language]
                    });
                    return;
                }
            ...
            $(document).on('click', '#add-weapons', function() {
                ...
                $('.inventoryItemSelected').each(function() {
                    ...
                    var inventoryID = parseInt($(this).data('id'));
                    var weapon = fbInventory.convertWeapon(getWeapon(inventoryID));
                    tradeWeapons.push(weapon);
                    
                    $(this).clone().removeClass('inventoryItemSelected').appendTo('#my-trades__your-offer .trade__info__weapons.your').data('weapon_obj', weapon).find('i').remove();
                        ...
                        fbProfile.tradeInfo(tradeID, function(tradeInfo) {
                            ...
                            for (var i = 0; i < convertedWeapons.length; i++) {
                                if (isAndroid())
                                    ...
                                    inventory.push(convertedWeapons[i]);
                            }
                            if (!isAndroid())
                                saveInventory();
